...
yASCII_tie_full         (short bx, short by, short ex, short ey, char a_tall, char a_blane, char a_vlane, char a_elane)
{
   ...
   else if (a_blane == a_tall - 1)   yASCII_print (bx, by + a_tall , "à", YASCII_CLEAR); 
   else                              yASCII_print (bx, by + a_blane, "á", YASCII_CLEAR); 
   /*---(finish)-------------------------*/
   if      (a_elane == 0)            yASCII_print (ex, ey          , "â", YASCII_CLEAR); 
   else if (a_elane == a_tall - 1)   yASCII_print (ex, ey + a_tall , "à", YASCII_CLEAR); 
   ...
   case '‘' : 
      ...
      yASCII_single (ex - a_vlane, by + a_blane, 'Ö');
      for (i = by + a_blane - 1; i >= ey + a_elane + 1; --i)  yASCII_single (ex - a_vlane, i, 'å');
      yASCII_single (ex - a_vlane, ey + a_elane, 'É');
      break;
   case '’' :
